(function () {
  ...
  async function load() {
    try {
      var data = await AppUtils.fetchJSON(API + "/api/security/findings");
      renderSummary(data);
  ...
  function renderFindings(findings) {
    ...
    findingsEl.addEventListener("click", function (e) {
      ...
      if (key) CardUtils.openUserCardPopup(key, link.textContent.trim());
    });
  }
  ...
  CardUtils.setCardOpener(function (key, name) {
    CardUtils.openUserCardPopup(key, name);
  });
